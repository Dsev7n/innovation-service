<template>
  <div id="company-project-publish">
    <p class="company-project-publish-guide">项目>发布项目</p>
    ...
    <form action="/" method="post" id="proForm">
      ...
      <label for="proMoney" class="peoMoney-label"><p>项目金额</p><select name="proMoney" id="proMoney" class="project-select"></select></label>
    ...
    <div class="confirm-background" :class="{buttonShow:isShow}">
...
#company-project-publish {
  display: grid;
  grid-template-rows: 40px 50px 69px 1fr 80px 1fr;
  background-color: #eee;
  height: 100vh;
}
.company-project-publish-guide {
  font-size: 18px;
  color: #777;
  grid-row: 2/3;
  margin-left:50px;
...
#proForm {
  ...
  grid-template-rows: 80px 80px 140px 80px 140px;
